...
#endif
		while( !stopping_condition( point, first_derivative ) ) {
...
#endif
			for(
				evaluator.evaluate( point + lambda * h, 0 ) ;
				( evaluator.get_value_of_function() - current_function_value ) > ( line_search_tolerance * lambda * directional_derivative ) ;
				evaluator.evaluate( point + lambda * h, 0 )
			) {
#if DEBUG_NEWTON_RAPHSON
				std::cerr << "MNR: trying line search with lambda=" << lambda << ".\n" ;
#endif
				lambda = lambda * 0.5 ;
			}
			
			std::cerr << "MNR: line search complete with lambda = " << lambda << " and improvement " << ( evaluator.get_value_of_function() - current_function_value ) << ".\n" ;
			
			point += lambda * h ;
			evaluator.evaluate_at( point ) ;
#if DEBUG_NEWTON_RAPHSON
			std::cerr << "MNR: point = " << point << ", function = " << function_value << ", first derivative = " << first_derivative.transpose() << ".\n" ;
#endif
