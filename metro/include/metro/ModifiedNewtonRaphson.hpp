...
namespace metro {
	...
	// A 'stepper' object is used to tell the algorithm whether to take another step,
	...
	// If with nonpositive directional derivative is picked then the algorithm gamely tries to
	// improve the function or derivative anyway, but will stay at the same point if it cannot
...
//			function.evaluate_at( currentPoint, 1 ) // needs likelihood optimisation
			...
			if( directional_derivative > ( 10 * std::numeric_limits< double >::epsilon() ) ) {
				...
				// We can't reasonably expect to improve the function much if the directional derivative
				// is so small.  Instead we stop as long as the function gets no worse
				// and the derivative gets no smaller.
				...
				std::cerr << "MNR: tried line search with lambda=" << lambda
					...
					<< ": target 1-norm = " << derivativeOneNorm
					<< ".\n" ;
	#endif
					lambda = lambda * 0.5 ;
					if( lambda < std::numeric_limits< double >::epsilon() ) {
