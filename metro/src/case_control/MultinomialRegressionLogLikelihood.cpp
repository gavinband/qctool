...
namespace metro {
	namespace case_control {
		...
		MultinomialRegressionLogLikelihood::MultinomialRegressionLogLikelihood(
			...
			m_design( design ),
			m_number_of_outcomes( number_of_outcomes ),
			m_number_of_samples( m_design->outcome().size() ),
			m_parameter_vector( Vector::Zero( m_design->matrix().cols() * ( m_number_of_outcomes - 1 ) )),
			m_parameter_matrix( Matrix::Zero( m_design->matrix().cols(), m_number_of_outcomes - 1 ) ),
		...
		void MultinomialRegressionLogLikelihood::compute_psi(
			...
			assert( result != 0 ) ;
			result->setZero( m_number_of_samples, m_number_of_outcomes * number_of_predictor_levels ) ;
		...
		void MultinomialRegressionLogLikelihood::compute_rearranger( int const number_of_predictor_levels, PermutationMatrix* result ) const {
			...
			result->setIdentity(
				number_of_predictor_levels * m_number_of_outcomes
			) ;
			int column = 0 ;
			for( int j = 0; j < m_number_of_outcomes; ++j ) {
				for( int level_i = 0; level_i < number_of_predictor_levels; ++level_i, ++column ) {
//					result->indices().data()[( level_i * m_number_of_outcomes + j )] = column ;
					result->indices().data()[ column ] = ( level_i * m_number_of_outcomes ) + j ;
				}
			}
#if DEBUG_LOGLIKELIHOOD			
		...
		LogLikelihood::IntegerMatrix MultinomialRegressionLogLikelihood::compute_parameter_identity() const {
			...
			for( int j = 0; j < D; ++j ) {
				for( int i = 1; i < m_number_of_outcomes; ++i ) {
					result( (i-1)*D + j, 0 ) = i ;
					result( (i-1)*D + j, 1 ) = j ;
				}
			}
		...
		void MultinomialRegressionLogLikelihood::evaluate_at_impl(
			...
			m_parameter_matrix.resize( m_design->matrix().cols(), m_number_of_outcomes - 1 ) ; 
		...
		void MultinomialRegressionLogLikelihood::compute_F( Matrix const& parameters, Matrix* result ) const {
			...
			for( int level_i = 0; level_i < number_of_levels; ++level_i ) {
				...
				MatrixBlock alternative_outcome_block = result->block( 0, block_first_column + 1, m_number_of_samples, m_number_of_outcomes - 1 ) ;
		...
		void MultinomialRegressionLogLikelihood::compute_A_and_function_value(
			...
			*value_of_function = 0.0 ;
			...
			for( std::size_t i = 0; i < included_samples.size(); ++i ) {
				...
				int const end_row = included_samples[i].end() ;
				for( int sample = start_row; sample < end_row; ++sample ) {
					int const outcome_i = m_design->outcome()( sample ) ;
