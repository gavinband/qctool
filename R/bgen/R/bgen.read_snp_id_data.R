...
require( bitops ) ;
    ...
    data = list()
    data$number_of_samples = readBin( con, integer(), size = 4, n = 1, endian = endian )

if( length( data$number_of_samples ) == 0 ) {
return( NULL ) ;
}

    id_storage_size = readBin( con, integer(), size = 1, n = 1, endian = endian, signed = FALSE )

    SNPID_size = readBin( con, integer(), size = 1, n = 1, endian = endian, signed = FALSE )
    data$SNPID = readChar( con, nchars = SNPID_size )
    if( id_storage_size > SNPID_size ) {
        readChar( con, nchar = id_storage_size - SNPID_size )
    }

    rsid_size = readBin( con, integer(), size = 1, n = 1, endian = endian, signed = FALSE )
    data$rsid = readChar( con, nchars = rsid_size )
    if( id_storage_size > rsid_size ) {
        readChar( con, nchar = id_storage_size - rsid_size )
    }
 
    chromosome = readBin( con, integer(), size = 1, n = 1, endian = endian, signed = FALSE )
    if( chromosome < 23 ) {
    ...
    data$position = readBin( con, integer(), size = 4, n = 1, endian = endian )

if( bitAnd( header$flags, 2 ) ) {
alleleA_size = readBin( con, integer(), size = 1, n = 1, endian = endian, signed = FALSE ) ;
data$alleleA = readChar( con, nchars = alleleA_size ) ;
alleleB_size = readBin( con, integer(), size = 1, n = 1, endian = endian, signed = FALSE ) ;
