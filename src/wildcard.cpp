...
namespace wildcard {
	...
	find_matches_for_paths_with_integer_wildcard(
		...
		for( std::size_t i = 0; i < filenames.size(); ++i ) {
			std::vector< FilenameMatch > these_matches = find_matches_for_path_with_integer_wildcard(
				filenames[i], wildcard_char, match_lower_bound, match_upper_bound
			) ;
			result.insert( result.end(), these_matches.begin(), these_matches.end() ) ;
		}
		return result ;
	}
