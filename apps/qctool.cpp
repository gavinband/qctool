...
*/
		...
		options[ "-osnp" ]
			.set_description( "Set the name of the file used to output results of per-SNP computations." )
			.set_takes_single_value() ;

		options[ "-osample" ]
			.set_description( "Set the name of the file used to output results of per-sample computations." )
			.set_takes_single_value() ;
		...
		options[ "-bgen-permitted-input-rounding-error" ]
			.set_description(
				...
				"E.g. if data is accurate to 3 decimal places, a value of 0.0005 should work here, while if "
				"your data is less accurate you may need to relax this.  BGEN v1.2 format rescales genotype "
				"probabilities so that they sum to 1; this implies "
...
private:
	...
	typedef std::map< genfile::VariantIdentifyingData, genfile::VariantIdentifyingData, genfile::VariantIdentifyingData::CompareFields > SNPDictionary ;
...
private:
	...
	void setup() {
		...
		if( m_options.check_if_option_has_value( "-map-id-data" )) {
			m_snp_dictionary = load_snp_dictionary( m_options.get_value< std::string >( "-map-id-data" ) ) ;
		}
...
private:
	...
	void unsafe_process() {
		...
		qcdb::Storage::SharedPtr per_snp_storage ;
		if( SNPSummaryComponent::is_needed( options() )) {
			if( !options().check( "-osnp" )) {
				throw genfile::BadArgumentError(
		...
		if( SampleSummaryComponent::is_needed( options() ) ) {
			...
			sample_summary_component->setup( processor, per_sample_storage ) ;
		}
