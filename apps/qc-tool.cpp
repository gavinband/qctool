...
struct QCToolOptionProcessor: public CmdLineOptionProcessor
{
	...
	std::string const& input_sample_filename() const { return m_sample_filename ; }
	std::string const& output_sample_filename() const { return m_output_sample_filename ; }
	std::string const& sample_stats_output_filename() const { return m_sample_statistic_filename ; }
...
private:
	...
	void get_sample_related_filenames() {
		if( check_if_option_was_supplied( "-s" ) ) {
			m_sample_filename = get_value< std::string >( "-s" ) ;
		}
		...
		if( check_if_option_was_supplied( "-sample-stats" ) ) {
			...
			if( check_if_option_was_supplied( "-os" )) {
				...
				m_output_sample_filename = m_sample_filename ;
			}
		}
		...
		if( check_if_option_was_supplied_in_group( "Sample filtering options" )) {
			if( check_if_option_was_supplied( "-os" )) {
				...
				m_output_sample_filename = strip_sample_file_extension_if_present( m_sample_filename ) + ".fltrd.sample" ;
			}
		}
	}
...
struct QCToolContext
{
	...
	virtual ObjectSource< SampleRow >& sample_source() const = 0 ;
	virtual ObjectSink< SampleRow >& fltrd_in_sample_sink() const = 0 ;
	virtual ObjectSink< SampleRow >& fltrd_out_sample_sink() const = 0 ;
	virtual std::ostream& snp_stats_file() const = 0 ;
	virtual std::ostream& sample_stats_file() const = 0 ;
...
struct QCToolCmdLineContext: public QCToolContext
{
	...
	ObjectSource< SampleRow >& sample_source() const {
		return *m_sample_source ;
	}
	...
	std::ostream& snp_stats_file() const {
		assert( m_snp_stats_file.get() ) ;
		return *m_snp_stats_file ;
	}

	std::ostream& sample_stats_file() const {
...
private:
	...
	void open_gen_row_sink() {
		...
		reset_filtered_out_gen_row_sink() ;
		open_filtered_out_gen_row_sink() ;
	}
	...
	void reset_filtered_out_gen_row_sink() {
	}

	void open_filtered_out_gen_row_sink() {
	}
	...
	void open_sample_stats_file() {
		if( m_options.sample_stats_output_filename() != "" ) {
			m_sample_stats_file = open_file_for_output( m_options.sample_stats_output_filename() ) ;
		}
	}
	
	void reset_sample_stats_file() {
...
private:
	...
	void open_log_file() {
		...
		m_backup_creator.backup_file_if_necessary( m_log_filename ) ;
	...
	void open_m_gen_row_source() {
		...
		for( std::size_t i = 0; i < m_gen_file_mapper.input_files().size(); ++i ) {
			Timer file_timer ;
			m_logger << "(Opening gen file \"" << m_gen_file_mapper.input_files()[i] << "\"...)" << std::flush ;
			try {
		...
		if( timer.elapsed() > 1.0 ) {
			m_logger << "Opened " << m_gen_file_mapper.input_files().size() << " GEN files in " << timer.elapsed() << "s.\n" ;\
		}
	}
	...
	void move_to_next_output_file( std::size_t index ) {
		if( index < m_gen_file_mapper.input_files().size() ) {
			if( m_gen_file_mapper.output_filenames().size() > 0 ) {
				if( m_gen_file_mapper.index_of_filename_corresponding_to( index ) != m_fltrd_in_snp_data_sink->index_of_current_sink() ) {
					m_fltrd_in_snp_data_sink->move_to_next_sink() ;
				}
			}
			
			if( m_snp_stats_file_mapper.output_filenames().size() > 0 ) {
				if( m_snp_stats_file_mapper.index_of_filename_corresponding_to( index ) != m_current_snp_stats_filename_index ) {
					open_gen_stats_file( ++m_current_snp_stats_filename_index, m_snp_statistics ) ;
				}
			}
		}
	}
	...
	void open_sample_row_source() {
		...
		if( m_sample_filename != "" ) {
			m_sample_source.reset( new SampleInputFile< SimpleFileObjectSource< SampleRow > >( open_file_for_input( m_sample_filename ))) ;
		}
	} ;

	void open_sample_row_sink() {
		m_fltrd_in_sample_sink.reset( new NullObjectSink< SampleRow >() ) ;
		if( m_output_sample_filename != "" ) {
			m_fltrd_in_sample_sink.reset( new SampleOutputFile< SimpleFileObjectSink< SampleRow > >( open_file_for_output( m_output_sample_filename ))) ;
		}
	}
	...
	void construct_snp_statistics() {
		...
		GenRowStatisticFactory::add_statistics( row_statistics_specs, m_snp_statistics ) ;
	}
	...
	void write_preamble() {
		...
		try {
			...
			for( std::size_t i = 0; i < m_gen_file_mapper.input_files().size(); ++i ) {
				...
				m_logger << "  (" << std::setw(6) << m_gen_row_source->number_of_snps_in_source( i ) << " snps)  " ;
				m_logger << "\"" << m_gen_file_mapper.input_files()[i] << "\"" ;				
				m_logger << "\n" ;
			}
			m_logger << std::string( 30, ' ' ) << "  (total " << m_gen_row_source->total_number_of_snps() << " snps in input).\n" ;
			...
			if( m_gen_file_mapper.output_filenames().empty() ) {
				...
				for( std::size_t i = 0; i < m_gen_file_mapper.output_filenames().size(); ++i ) {
					...
					m_logger << "  \"" << m_gen_file_mapper.output_filenames()[i] << "\"" ;				
				}
				m_logger << "\n" ;
			}
			...
			for( std::size_t i = 0; i < m_snp_stats_file_mapper.output_filenames().size(); ++i ) {
				...
				m_logger << "  \"" << m_snp_stats_file_mapper.output_filenames()[i] << "\"\n" ;
			}
			m_logger << "\n" ;
...
private:
	...
	void unsafe_preprocess_sample_rows() {
		if( m_sample_filename != "" ) {
			...
			for( ; (*m_sample_source) >> sample_row; ++m_number_of_sample_file_rows ) {
	...
	void check_for_errors() {
		...
		for( std::size_t i = 0; i < m_gen_file_mapper.input_files().size(); ++i ) {
			...
			if( strings_are_nonempty_and_equal( m_output_sample_filename, m_gen_file_mapper.input_files()[i] )) {
				m_errors.push_back( "Output SAMPLE file \"" + m_output_sample_filename +"\" also specified as input GEN file." ) ;
				break ;
			}
		...
		for( std::size_t j = 0; j < m_gen_file_mapper.output_filenames().size(); ++j ) {
			if( strings_are_nonempty_and_equal( m_output_sample_filename, m_gen_file_mapper.output_filenames()[j] )) {
	...
	void check_for_warnings() {
		if( (m_output_sample_filename != "" || m_sample_statistic_filename != "") && m_gen_file_mapper.input_files().size() != 23 ) {
		...
		if( m_gen_file_mapper.output_filenames().size() == 0 && m_output_sample_filename == "" && m_snp_stats_file_mapper.output_filenames().size() == 0 && m_sample_statistic_filename == "" ) {
		...
		if(m_output_sample_filename == "" && (m_sample_filter->number_of_subconditions() > 0)) {
			m_warnings.push_back( "You have not specified sample filters, but no output sample file.\n" ) ;
		}
		if( strings_are_nonempty_and_equal( m_output_sample_filename, m_sample_filename )) {
	...
	void process_gen_rows() {
		for( std::size_t i = 0; i < m_gen_file_mapper.output_filenames().size(); ++i ) {
			m_backup_creator.backup_file_if_necessary( m_gen_file_mapper.output_filenames()[i] ) ;
		}
		for( std::size_t i = 0; i < m_snp_stats_file_mapper.output_filenames().size(); ++i ) {
			m_backup_creator.backup_file_if_necessary( m_snp_stats_file_mapper.output_filenames()[i] ) ;
		}
		m_logger << "Processing SNPs...\n" ;
		Timer timer ;
		open_gen_row_sink() ;
		if( m_snp_stats_file_mapper.output_filenames().size() > 0 ) {
			open_gen_stats_file( 0, m_snp_statistics ) ;
		}
		...
		m_logger << "Processed " << m_gen_row_source->total_number_of_snps() << " SNPs in "
			<< std::fixed << std::setprecision(1) << timer.elapsed() << " seconds.\n" ;
		if( m_snp_filter->number_of_subconditions() > 0 ) {
			m_logger << "(" << m_number_of_filtered_in_snps << " of " << m_gen_row_source->total_number_of_snps() << " SNPs passed the filter.)\n" ;
		}
		/*
		m_logger << "Post-processing..." << std::flush ;
		timer.restart() ;
	...
	void open_gen_stats_file( std::size_t index, GenRowStatistics const& row_statistics ) {
		...
		if( genStatisticOutputFile.get() ) { 
			*genStatisticOutputFile << "        " ;
			m_snp_statistics.format_column_headers( *genStatisticOutputFile ) << "\n";
		}
	...
	void process_sample_rows() {
		m_logger << "Processing samples...\n" ;
		Timer timer ;

		apply_sample_filter() ;
		assert( m_sample_rows.size() == m_per_column_amounts.size() ) ;

		m_backup_creator.backup_file_if_necessary( m_output_sample_filename ) ;
		m_backup_creator.backup_file_if_necessary( m_sample_statistic_filename ) ;
		...
		for( std::size_t i = 0 ; i < m_per_column_amounts.size(); ++i ) {
			...
			(*m_fltrd_in_sample_sink) << sample_row ;
		}

		m_logger << "Processed " << m_gen_row_source->number_of_samples() << " samples in " << std::fixed << std::setprecision(1) << timer.elapsed() << " seconds.\n" ;
		if( m_sample_filter->number_of_subconditions() > 0 ) {
			m_logger << "(" << m_sample_rows.size() << " of " << m_gen_row_source->number_of_samples() << " samples passed the filter.)\n" ;
		}
		m_logger << "\n" ;
	}
	...
	void process_gen_row( GenRow const& row, std::size_t row_number, std::size_t& m_number_of_filtered_in_snps ) {
		m_snp_statistics.process( row ) ;
		if( m_snp_filter->check_if_satisfied( m_snp_statistics )) {
			++m_number_of_filtered_in_snps ;
			output_gen_row( row ) ;
			output_gen_row_stats( m_snp_statistics, row_number ) ;
		}
		else {
			do_snp_filter_diagnostics( m_snp_statistics, row_number ) ;
		}
	}

	void output_gen_row( GenRow const& row ) {
		row.write_to_sink( *m_fltrd_in_snp_data_sink ) ;
	}

	void output_gen_row_stats( GenotypeAssayStatistics const& row_statistics, std::size_t row_number ) {
		if( genStatisticOutputFile.get() ) {
			if( m_snp_statistics.size() > 0 ) {
				(*genStatisticOutputFile)
