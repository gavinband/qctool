...
public:
	...
	void declare_options( appcontext::OptionProcessor& options ) {
		...
		options[ "-output-models" ]
			...
			.set_check(
					[]( std::string const& arg, std::vector< std::string > const& values ) -> bool {
						for( auto s: values ) {
							if( s != "alternative" && s != "all" ) {
								throw appcontext::OptionValueInvalidException(
									arg,
									values,
									arg + ": unexpected value (expected \"genetic\" or \"all\")."
								) ;
							}
						}
						return true ;
...
private:
	...
	void unsafe_process() {
		...
		genfile::SNPDataSource::UniquePtr
			host = open_snp_data_sources( options().get< std::string >( "-predictor" )) ;
		genfile::SNPDataSource::UniquePtr
			para = open_snp_data_sources( options().get< std::string >( "-outcome" )) ;
