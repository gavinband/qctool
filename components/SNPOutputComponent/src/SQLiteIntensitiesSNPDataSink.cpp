...
SQLiteIntensitiesSNPDataSink::SQLiteIntensitiesSNPDataSink( qcdb::DBOutputter::UniquePtr outputter ):
...
std::string SQLiteIntensitiesSNPDataSink::get_spec() const {
	return "SQLiteIntensitiesSNPDataSink" ;
}
...
namespace {
	...
	struct IntensityWriter: public genfile::VariantDataReader::PerSampleSetter {
		enum Encoding { eBITPACK = 0 } ; // must take contiguous values starting at zero.
		
		IntensityWriter( std::vector< char >* compression_buffer ):
		...
		void set_number_of_samples( std::size_t n ) {
...

			{
				...
				std::string const encoding_name = "2float" ;
				m_buffers[ eBITPACK ].resize( 3 + encoding_name.size() + 5 + 3 + ( n * 4 * 2 ) ) ; // name, plus 2 intensities stored as floats, per sample.
...
void SQLiteIntensitiesSNPDataSink::set_sample_names_impl( std::size_t number_of_samples, SampleNameGetter getter ) {
...
void SQLiteIntensitiesSNPDataSink::write_variant_data_impl(
	...
	IntensityWriter writer( &(m_data[ m_data_i ]) ) ;
	data_reader.get( "intensities", writer ) ;
	writer.finalise() ;
	++m_data_i ;
}

void SQLiteIntensitiesSNPDataSink::finalise_impl() {
	if( m_data_i > 0 ) {
		flush_data( m_data_i ) ;
		m_data_i = 0 ;
	}
...
void SQLiteIntensitiesSNPDataSink::flush_data( std::size_t const data_count ) {
	db::Connection::ScopedTransactionPtr transaction = m_outputter->connection().open_transaction( 600 ) ;
