...
AUTO_TEST_CASE( test_fixed_number_entry_type ) {
		...
		for( std::size_t ploidy = 0; ploidy < 10; ++ploidy ) {
			for( std::size_t n_alleles = 0; n_alleles < 10; ++n_alleles ) {
				Result r, rp ;
				{
					...
					r = type.parse( "", n_alleles ) ;
					rp = type.parse( "", n_alleles, ploidy ) ;
					TEST_ASSERT( r == rp ) ;
					TEST_ASSERT( r.size() == 0 ) ;
				}
				{
					FixedNumberVCFEntryType type( 1, SimpleType::create( "Integer" ) ) ;
					r = type.parse( "1", n_alleles ) ;
					rp = type.parse( "1", n_alleles, ploidy ) ;
					TEST_ASSERT( r == rp ) ;
					TEST_ASSERT( r.size() == 1 ) ;
					TEST_ASSERT( r[0] == Entry( 1 )) ;

					r = type.parse( "5", n_alleles ) ;
					rp = type.parse( "5", n_alleles, ploidy ) ;
					TEST_ASSERT( r == rp ) ;
					TEST_ASSERT( r.size() == 1 ) ;
					TEST_ASSERT( r[0] == Entry( 5 )) ;

					r = type.parse( "-1000", n_alleles ) ;
					rp = type.parse( "-1000", n_alleles, ploidy ) ;
					TEST_ASSERT( r == rp ) ;
					TEST_ASSERT( r.size() == 1 ) ;
...

				{
					...
					r = type.parse( "1,2,3,4,5", n_alleles ) ;
					rp = type.parse( "1,2,3,4,5", n_alleles, ploidy ) ;
					TEST_ASSERT( r == rp ) ;
					...
					r = type.parse( "5,4,.,0,-1", n_alleles ) ;
					rp = type.parse( "5,4,.,0,-1", n_alleles, ploidy ) ;
					TEST_ASSERT( r == rp ) ;
					TEST_ASSERT( r.size() == 5 ) ;
					TEST_ASSERT( r[0] == Entry( 5 )) ;
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( "", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( ",", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "1", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "-1", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( ".", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( ".,.", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,3", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}

		try { type.parse( "1,2.,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "1,2.,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "3.5", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "3.5", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
	}
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( "", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( ",", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "2", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "-1", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( ".", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "1,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,.", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( ".,1", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( ".,.", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "-1,-1", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,3", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "-1,.,-2000", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		...
		try { type2.parse( "1,2.,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "3.5", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "3.5", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
	}
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( "-1", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( ".", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "1,2", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( ".,.", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "-1,-1", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "1,2,3", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "-1,.,-2000", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( ".,.", n_alleles ) ; TEST_ASSERT(0) ;  } catch( BadArgumentError const& ) { }
		try { type.parse( "-1,-1", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,3", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "1,.,2", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "-1,.,-2000", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "A,.,2", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type2.parse( "1,2-3,2", n_alleles ) ;  } catch( BadArgumentError const& ) { TEST_ASSERT(0) ;}
		try { type2.parse( "1,2,", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( "1,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {  }
		try { type.parse( "-1,.,-2000", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "1,.,2,3,3,50,50,.,76,.", n_alleles ) ; } catch( BadArgumentError const& ) { TEST_ASSERT(0) ; }
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( "1,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {  }
		try { type.parse( "-1,.,-2000", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "1,.,2,3,3,50,50,.,76,.", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) { }
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
...
AUTO_TEST_CASE( test_one_per_alternate_allele_entry_type ) {
...

	{
		...
		for( std::size_t ploidy = 0; ploidy < 100; ++ploidy ) {
			r = type.parse( "", 0 ) ;
			rp = type.parse( "", 0, ploidy ) ;
			TEST_ASSERT( r == rp ) ;
			TEST_ASSERT( r.size() == 0 ) ;

			r = type.parse( "5,6", 2 ) ;
			rp = type.parse( "5,6", 2, ploidy ) ;
			TEST_ASSERT( r == rp ) ;
	...
	for( std::size_t n_alleles = 0; n_alleles < 100; ++n_alleles ) {
		...
		try { type.parse( "", n_alleles ) ; TEST_ASSERT( n_alleles == 0 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 0 ) ; }
		try { type.parse( ",", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1", n_alleles ) ; TEST_ASSERT( n_alleles == 1 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 1 ) ; }
		try { type.parse( "2", n_alleles ) ; TEST_ASSERT( n_alleles == 1 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 1 ) ; }
		try { type.parse( "-1", n_alleles ) ; TEST_ASSERT( n_alleles == 1 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 1 ) ; }
		try { type.parse( ".", n_alleles ) ; TEST_ASSERT( n_alleles == 1 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 1 ) ; }
		try { type.parse( "1,2", n_alleles ) ; TEST_ASSERT( n_alleles == 2 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 2 ) ; }
		try { type.parse( ".,.", n_alleles ) ; TEST_ASSERT( n_alleles == 2 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 2 ) ; }
		try { type.parse( "-1,-1", n_alleles ) ; TEST_ASSERT( n_alleles == 2 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 2 ) ; }
		try { type.parse( "1,2,3", n_alleles ) ; TEST_ASSERT( n_alleles == 3 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 3 ) ; }
		try { type.parse( "1,.,2", n_alleles ) ; TEST_ASSERT( n_alleles == 3 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 3 ) ; }
		try { type.parse( "-1,.,-2000", n_alleles ) ; TEST_ASSERT( n_alleles == 3 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 3 ) ; }
		try { type.parse( "1,.,2,3,3,50,50,.,76,.", n_alleles ) ; TEST_ASSERT( n_alleles == 10 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 10 ) ; }
		try { type.parse( "A,.,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type.parse( "1,2,", n_alleles ) ; TEST_ASSERT(0) ; } catch( BadArgumentError const& ) {}
		try { type2.parse( "A,.,2", n_alleles ) ; TEST_ASSERT( n_alleles == 3 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 3 ) ; }
		try { type2.parse( "1,2-3,2", n_alleles ) ; TEST_ASSERT( n_alleles == 3 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 3 ) ; }
		try { type2.parse( "1,2,", n_alleles ) ; TEST_ASSERT( n_alleles == 3 ) ; } catch( BadArgumentError const& ) { TEST_ASSERT( n_alleles != 3 ) ; }
	}
	std::cerr << "ok.\n" ;
}
