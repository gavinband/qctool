...
namespace genfile {
	namespace vcf {
		...
		ThreshholdingGenotypeSetter< Eigen::VectorXf >::ThreshholdingGenotypeSetter(
		...
		void ThreshholdingGenotypeSetter< Eigen::VectorXf >::set_number_of_samples( std::size_t n ) {
			m_result.setConstant( n, m_missing_value ) ;
			if( m_non_missingness ) {
				m_non_missingness->setConstant( n, 0 ) ;
			}
		...
		void ThreshholdingGenotypeSetter< Eigen::VectorXf >::set( std::size_t sample_i, double AA, double AB, double BB ) {
			if( AA > m_threshhold ) {
				m_result( sample_i ) = m_AA_value ;
				if( m_non_missingness ) {
					(*m_non_missingness)( sample_i ) = 1 ;
