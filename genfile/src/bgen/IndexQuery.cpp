...
namespace genfile {
	namespace bgen {
		IndexQuery::UniquePtr IndexQuery::create(
			...
			return IndexQuery::UniquePtr( new SqliteIndexQuery( filename, query, callback, table_name )) ;
		}

		SqliteIndexQuery::SqliteIndexQuery(
			std::string const& filename,
			Query const& query,
			SqliteIndexQuery::ProgressCallback callback,
			std::string const& table_name
		):
			...
			m_initialised( false )
		{
			implement_query( query, callback ) ;
		}
		
		void SqliteIndexQuery::implement_query(
			Query const& query,
			SqliteIndexQuery::ProgressCallback callback
		) {
			include_ranges( query.included_ranges() ) ;
			exclude_ranges( query.excluded_ranges() ) ;
		...
		SqliteIndexQuery& SqliteIndexQuery::include_ranges( std::vector< GenomicRange > const& ranges ) {
			for( std::size_t i = 0; i < ranges.size(); ++i ) {
				include_range( ranges[i] ) ;
			}
			return *this ;
		}

		SqliteIndexQuery& SqliteIndexQuery::exclude_ranges( std::vector< GenomicRange > const& ranges ) {
			for( std::size_t i = 0; i < ranges.size(); ++i ) {
				exclude_range( ranges[i] ) ;
			}
			return *this ;
		}
